...
import '../styles/ktl-footer.css';

type SvgIcon = (props: SVGProps<SVGSVGElement>) => React.ReactNode;
...
const socialNetworks: SocialNetwork[] = [
  ...
  },
  {
    ...
    icon: (props) => (
      <svg width={16} height={19} fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
...
const footerNav: FooterNavItem[] = [
  ...
  { url: 'https://blog.jetbrains.com/kotlin/', title: 'Blog', isTargetBlank: true },
  { url: 'https://youtrack.jetbrains.com/issues/KT', title: 'Issue Tracker', isTargetBlank: true },
  {
    url: 'https://resources.jetbrains.com/storage/products/kotlin/docs/kotlin_logos.zip',
    title: 'Brand assets',
    isTargetBlank: true,
  },
  { url: 'https://www.jetbrains.com/careers/jobs/?team=Kotlin', title: 'Careers', isTargetBlank: true },
];

function SocialItem({ socialNetwork }: { socialNetwork: SocialNetwork }) {
  ...
  return (
    <a
      ...
      className={cn('ktl-social-item-module_social-item_LDEev', {
        'ktl-social-item-module_social-item-dark-theme_ZhWgN': theme === 'dark',
      })}
...
function NavItem({ data }: { data: FooterNavItem }) {
  const textCn = useTextStyles();
  const theme = useTheme();

  return (
    <li
      className={cn('ktl-nav-item-module_nav-item_526AM', {
        'ktl-nav-item-module_nav-item-dark-theme__ymom': theme === 'dark',
      })}
    >
      <a
        target={data.isTargetBlank ? '_blank' : undefined}
        rel={data.isTargetBlank ? 'noopener noreferrer' : undefined}
        href={data.url}
        title={data.title}
        className={cn(textCn('rs-text-2'), textCn('rs-link', { hardness: 'average', mode: 'nude' as never }))}
      >
        {data.title}
      </a>
    </li>
  ...
  return (
    <nav className={className}>
      <ul className="ktl-nav-list-module_nav-list_2YPZ6">
        {footerNav.map((item) => (
          <NavItem key={item.url} data={item} />
        ))}
      </ul>
    </nav>
  );
...
function JetBrainsLogo(props: SVGProps<SVGSVGElement>) {
  return (
...
function JetBrainsLogoLink({ className }: { className?: string }) {
  return (
    <a href="https://jetbrains.com" target="_blank" rel="noopener noreferrer" className={cn(className, 'ktl-logo-module_link_3Bppl')}>
      <JetBrainsLogo />
    </a>
  );
}

const underlinedLinkClassName = 'ktl-footer-module_underlined-link_Pu1QP';

export function SiteGlobalFooter() {
  const textCn = useTextStyles();
  const theme = useTheme();
  const footerLinkClassName = textCn('rs-link', { hardness: 'pale', mode: 'nude' as never });

  return (
    <footer
      className={cn('ktl-footer-module_footer_3ENKj', {
        'ktl-footer-module_footer-dark-theme_1lVqh': theme === 'dark',
      })}
    >
      <div className="ktl-footer-module_social-list-area_3BpCr">
        <div className={cn(textCn('rs-h4'), 'ktl-footer-module_social-list-title_2m4tY')}>Stay in touch:</div>
        <SocialList />
      </div>

      <FooterNav className="ktl-footer-module_nav-list-area_11ftH" />

      <div className={cn(textCn('rs-text-3', { hardness: 'pale' }), 'ktl-footer-module_text-area_1SpnS')}>
        <div className="ktl-footer-module_copyright_2uBX0">
          Kotlin™ is protected under the{' '}
          <a
            className={cn(footerLinkClassName, underlinedLinkClassName)}
        ...
        <div className="ktl-footer-module_motto_22kgv">
          ...
          <a className={cn(footerLinkClassName, underlinedLinkClassName)} href="https://www.jetbrains.com/" target="_blank" rel="noopener noreferrer">
            JetBrains
          </a>
        </div>
      </div>

      <JetBrainsLogoLink className="ktl-footer-module_logo-area_24Dg6" />
    </footer>
  );
}
