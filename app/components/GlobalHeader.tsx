  ...
  return (
    <header className="ktl-header-module_header-menu_3q6JB ktl-header-module_border_3kxUB">
      <div className="ktl-logo-large-module_container_2BfGV rs-text-3 rs-text-3_hardness_average rs-text-3_theme_dark">
        <a className="ktl-logo-large-module_link_2HVM-" href="/" aria-label="Kotlin Home Page">
        ...
        <a className="ktl-logo-large-module_version-tag_4KEoG" href="https://github.com/JetBrains/kotlin/releases/tag/v1.6.20" target="_blank" rel="noopener noreferrer">
          v1.6.20
        </a>
      </div>
      ...
      <nav className="ktl-horizontal-menu-module_nav_19XiR ktl-horizontal-menu-module_dark-theme_3x53H">
        <ul className="ktl-horizontal-menu-module_menu-list_3f1cp">
          ...
          >
            <a
              className={`ktl-horizontal-menu-module_menu-item_1GBle rs-text-2 rs-text-2_theme_dark${isSolutionsOpen ? ' ktl-horizontal-menu-module_expanded_3BdiD' : ''}`}
              href="#"
              onClick={(event) => {
            ...
            <ul
              className={`ktl-horizontal-menu-module_dropdown-menu_YC90K rs-text-2 rs-text-2_theme_dark${
                isSolutionsOpen ? ' ktl-horizontal-menu-module_dropdown-menu-expanded_3s42o' : ''
              }`}
              style={{ display: isSolutionsOpen ? 'block' : undefined }}
              onMouseEnter={openSolutions}
              onMouseLeave={closeSolutionsWithDelay}
            >
              {solutionsItems.map((item) => (
                <li key={item.title}>
                  <a className="ktl-horizontal-menu-module_dropdown-menu-item_3DA-o" href={item.url}>
                    {item.title}
                  </a>
                </li>
              ))}
          ...
          {menuItems.map((item) => (
            <li key={item.title} className="ktl-horizontal-menu-module_menu-item-wrap_1iQfh">
              <a className="ktl-horizontal-menu-module_menu-item_1GBle rs-text-2 rs-text-2_theme_dark" href={item.url}>
                {item.title}
              </a>
            </li>
          ))}
      ...
      <button type="button" className="ktl-search-button_button_2B8BX" aria-label="Search">
