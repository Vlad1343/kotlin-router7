...
const sections = [
  ...
  },
  {
    ...
    description: (
      <>
        Share application logic between web, mobile, and desktop platforms while keeping an experience native to
        users.
        <br />
        <br />
...
function WhyKotlinContent({ tab, sort }: { tab: string; sort: 'default' | 'name' }) {
  ...
  return (
    <Section className="why-kotlin-section">
      <Container>
        ...
        <ProgrammingLanguage tab={tab} sort={sort} />
          ...
          <div key={index} className="kto-grid kto-grid-gap-32 kto-offset-top-96 kto-offset-top-md-48">
            ...
            <div className="kto-col-8 kto-col-md-12">
              {section.media === 'youtube' ? (
                ...
                <img src="/assets/images/index/multiplatform.svg" alt="Kotlin Multiplatform" className="why-kotlin-section__image" />
              )}
            </div>
          </div>
        ))}
...
export function WhyKotlinSection({ tab, sort }: { tab: string; sort: 'default' | 'name' }) {
  return (
    <ThemeProvider theme="light">
      <WhyKotlinContent tab={tab} sort={sort} />
    </ThemeProvider>
  );
}
